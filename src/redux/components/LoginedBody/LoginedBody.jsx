...
function LoginedBody() {
    ...
    const stars = new Array(duckdomData.length).fill(undefined);
    ...
    const random = async () => {
        const response = await axios.get(
            'http://54.180.87.103:4000/api/random',
            {
                headers: {
                    Authorization: `${accessToken}`,
                },
        ...
        if (response.status === 200) {
            ...
            if (receivedCountResponse.status === 200) {
                ...
                navigate(`/${randomReceiverId}`, { receivedCount }); // 랜덤 유저 방문
                setNickName(response.data.data.nickname); // 랜덤 유저의 닉네임 가져오기
                setRandomLength(receivedCount); // 랜덤 유저의 받은 글 갯수 설정
            } else {
